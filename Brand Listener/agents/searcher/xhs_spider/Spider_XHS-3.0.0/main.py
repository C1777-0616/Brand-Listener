    ...
    def spider_some_note(self, notes: list, cookies_str: str, base_path: dict, save_choice: str, excel_name: str = '', proxies=None):
        ...
        if save_choice == 'all' or save_choice == 'excel':
            ...
            try:
                ...
                # Excel/WPS 打开文件时会锁文件，回退到时间戳文件名避免任务失败。
                backup_file_path = os.path.abspath(
                    os.path.join(base_path['excel'], f'{excel_name}_{int(time.time())}.xlsx')
                )
                logger.warning(f'文件被占用，回退保存到: {backup_file_path}')
                save_to_xlsx(note_list, backup_file_path)
        ...
        try:
            ...
            if success:
                ...
                for note in notes:
                    ...
                    note_url = f"https://www.xiaohongshu.com/explore/{note_id}?xsec_token={note['xsec_token']}&xsec_source={xsec_source}"
                    note_list.append(note_url)
                    # 优先使用搜索接口返回的 note_card 直接组装，避免详情接口 code=-1 导致整表为空。
                    try:
                        note_data = dict(note)
            ...
            if save_choice == 'all' or save_choice == 'excel':
                excel_name = query
            # 如果搜索结果中没拿到可用 note_card，再走详情接口兜底。
            if len(note_info_list) == 0 and len(note_list) > 0:
                logger.warning('搜索结果未携带完整 note_card，回退使用详情接口补充数据')
                for note_url in note_list:
                    detail_success, detail_msg, note_info = self.spider_note(note_url, cookies_str, proxies)
                    if note_info is not None and detail_success:
                        note_info_list.append(note_info)
                logger.info(f'详情接口补充成功数量: {len(note_info_list)}')
            # 保存媒体
            if save_choice == 'all' or 'media' in save_choice:
            ...
            if save_choice == 'all' or save_choice == 'excel':
                ...
                try:
                    ...
                    logger.warning(f'文件被占用，回退保存到: {backup_file_path}')
                    save_to_xlsx(note_info_list, backup_file_path)
        except Exception as e:
            success = False
            msg = e
        logger.info(f'搜索关键词 {query} 笔记: {success}, msg: {msg}, 导出数量: {len(note_info_list)}')
        return note_list, success, msg
...
if __name__ == '__main__':
    ...
    # 1/2 示例任务（笔记链接、用户主页）默认不执行，避免过期链接导致干扰。
    # notes = [
    #     r'https://www.xiaohongshu.com/explore/683fe17f0000000023017c6a?xsec_token=xxx&xsec_source=pc_user',
    # ]
    # data_spider.spider_some_note(notes, cookies_str, base_path, 'all', 'test')
    # user_url = 'https://www.xiaohongshu.com/user/profile/xxxx?xsec_token=xxx&xsec_source=pc_feed'
